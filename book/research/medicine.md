...
Общий `sources.json` содержит 49 записей; 23 уникальных source IDs используются medical claims. Все 23 имеют `checked`. Текущие official sources WHO, EFSA, FDA, SAMR и регистрация ClinicalTrials.gov сохранены как guidance/supporting; peer-reviewed human studies и два pu-erh-specific reviews — supporting, потому что их applicability ограничена extract, biomarker или nonhuman design. Регистрация без результатов не входит в evidence of efficacy. Слабые blogs и popular summaries не используются как core medical evidence.
